...
/*   Updated: 2025/06/25 13:09:57 by ppontet          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
		...
		bool is_enabled)
{
	if (!mlx || !mlx->mlx_ptr || !win_ptr)
		return ;
	if (is_enabled == 1)
		mlx_put_image_to_window(mlx->mlx_ptr, win_ptr, mlx->circle_yes.ptr,
			element.x - element.radius, element.y - element.radius);
	else
		mlx_put_image_to_window(mlx->mlx_ptr, win_ptr, mlx->circle_no.ptr,
			element.x - element.radius, element.y - element.radius);
}
		...
		enum e_screen_size num, bool is_enabled)
{
	if (num == 0)
		...
		ft_put_circle_to_win(mlx, win_ptr, (t_circle){35, 140, 20,
			0x0}, is_enabled);
	else if (num == 2)
		ft_put_circle_to_win(mlx, win_ptr, (t_circle){35, 210, 20,
			0x0}, is_enabled);
	else if (num == 3)
		ft_put_circle_to_win(mlx, win_ptr, (t_circle){35, 280, 20,
			0x0}, is_enabled);
	else if (num == 4)
		ft_put_circle_to_win(mlx, win_ptr, (t_circle){35, 360, 20,
			0x0}, is_enabled);
}

void	ft_put_all_circle_to_win(t_mlx *mlx, void *win_ptr,
		bool is_enabled)
{
